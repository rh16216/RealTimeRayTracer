...
def createBasicMesh(corner, fullVec1, fullVec2, numDivides):
    ...
    meshGrid = torch.zeros(numDivides*numDivides, 3)
    for i in range(0, numDivides):
        for j in range(0, numDivides):
            meshGrid[i*numDivides + j] = corner + i*vec1 + j*vec2
...
def calculateFFGridPair(mesh1, mesh2, meshIndex1, meshIndex2):
    mesh1Size = mesh1.centreGrid.size()[0]
    mesh2Size = mesh2.centreGrid.size()[0]
    ffGrid = torch.zeros(mesh1Size, mesh2Size)

    for index1, vertex1 in enumerate(mesh1.centreGrid):
        for index2, vertex2 in enumerate(mesh2.centreGrid):
...
def projectToScreen(meshList, colours, width, height):
    ...
    for meshIndex, mesh in enumerate(meshList):
        for vertIndex, vertex in enumerate(mesh.meshGrid):
            ...
            vertex1 = Vertex(projectedVertex1X, projectedVertex1Y, colour0)
            vertex2 = Vertex(projectedVertex2X, projectedVertex2Y, colour0)
            vertex3 = Vertex(projectedVertex3X, projectedVertex3Y, colour0)
...
class FFNet(nn.Module):
    def __init__(self, ffGrid, numBounces):
        ...
        lightCorner = int(math.floor(wallSize + wallSize/2 - wallWidth + wallWidth/2 - 1))
...
with torch.no_grad():
    ...
    numPatches = 13*args.num_divides*args.num_divides
    ...
    pickleString = "ffGridPatches" + str(args.num_divides) + "BoxesShadows.pickle"
    ...
    model = FFNet(ffGrid, args.num_bounces).to(device=args.device)
